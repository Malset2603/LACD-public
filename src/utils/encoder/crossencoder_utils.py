...
class RuleCrossEncoderModelNew(torch.nn.Module):
    def __init__(self, model_name):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
        self.tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        self.rule_tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
...
class NLIDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # premise = "Does two articles conflict? [SEP]"+ premise
        ...
        if self.method == "case-augmentation":
            ...
            hypothesis += "\ncase:\n" + generate_case(None, None, hypothesis, case_idx=self.dataframe.iloc[index]['case_idx'])
            ...
            premise += "\nrule:\n" + generate_rule(None, None, premise, rule_idx=self.dataframe.iloc[index]['rule_idx'])
            hypothesis += "\nrule:\n" + generate_rule(None, None, hypothesis, rule_idx=self.dataframe.iloc[index]['rule_idx'])

        elif self.method == "rule-only":
            from src.methods.rule_augmentation.prompt import generate_rule
            premise = generate_rule(None, None, premise, rule_idx=self.dataframe.iloc[index]['rule_idx'])
            hypothesis = generate_rule(None, None, hypothesis, rule_idx=self.dataframe.iloc[index]['rule_idx'])

        # rule 과 article 을 별도로 encoding.
        elif self.method == "rule-augmentation-divide" or self.method == "rule-augmentation-divide-new":
            from src.methods.rule_augmentation.prompt import generate_rule

            rule_premise = generate_rule(None, None, premise, rule_idx=self.dataframe.iloc[index]['rule_idx'])
            rule_hypothesis = generate_rule(None, None, hypothesis, rule_idx=self.dataframe.iloc[index]['rule_idx'])
            ...
            premise_rules = generate_rule(None, None, premise, rule_idx=self.dataframe.iloc[index]['rule_idx']).split("IF:")[1:]
            hypothesis_rules = generate_rule(None, None, hypothesis, rule_idx=self.dataframe.iloc[index]['rule_idx']).split("IF:")[1:]
